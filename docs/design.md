...
18. update mode requires an existing selected-scope installation, uses update rather than install commands, skips missing/pinned dependencies, refuses cross-scope package ambiguity, and preserves files on dry-run, decline, or external failure; and
19. `bash -n` succeeds for each shell script individually.
