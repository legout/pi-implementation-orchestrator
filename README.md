...
- **Install-before-write ordering.** After approval, rendered outputs are staged in a unique temporary directory and external installs run **before** any project write. Prompt commands are copied to the scope-selected prompt directory after the external installs and before project writes; a failed copy exits nonzero and rerunning setup converges, like project writes. If an install fails, project files are untouched, the already-completed external steps are reported, and no destructive automatic uninstall is attempted. Each project output is then rendered to a same-directory temporary file and renamed into place. A write failure exits nonzero with a clear rerun instruction; earlier successful writes remain in place, and rerunning deterministically converges. Multi-file replacement and external installs are not one atomic transaction, and crash/power-loss durability is not promised.
